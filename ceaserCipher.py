...
output.writerow(['Pc Name', 'Key', 'Range',
                'Index(starting from 1)', 'Real Letter', 'Encrypted Message'])
for pc_num in data:
    key = randint(1, 25)
    current_range = get_range(key, range_size, 0, 25)
    en_msg = encrypt(pc_num, key)
    real_char_index = randint(0, len(pc_num)-1)
    real_char = pc_num[real_char_index]
    output.writerow([pc_num, key, current_range,
                    real_char_index+1, real_char, en_msg])
